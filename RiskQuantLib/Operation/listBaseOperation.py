...
class listBase():
    ...
    def fromDF(self, df : pd.DataFrame, code :str = '', name:str = '', tradingDate = '', belongTo = ''):
        """
        ...
        if not hasattr(self,'elementClass'):
            ...
            def trySetAttr(attrName,pdseries):
                try:
                    ...
                    pass
            [trySetAttr(col,df[col]) for col in df.columns if col not in ['code','Code']]
            indexCol = {attrName:colName for attrName,colName in zip(['tradingDate','belongTo'],[tradingDate,belongTo]) if colName!=''}
            [trySetAttr(key,df[indexCol[key]]) for key in indexCol]
            if code != '':
                [i.setCode(codeStr) for i,codeStr in zip(self,df[code])]
            elif 'code' in df.columns.tolist():
                [i.setCode(codeStr) for i, codeStr in zip(self, df['code'])]
            elif 'Code' in df.columns.tolist():
                [i.setCode(codeStr) for i, codeStr in zip(self, df['Code'])]
            else:
                pass
            if name != '':
                [i.setName(nameStr) for i, nameStr in zip(self, df[name])]
